...
class TradeController extends TradeFatherController
{
    ...
    private function trade($currencyId, $type, $num, $price)
    {
        ...
        switch ($type) {
            ...
            case 'buy':
                ...
                //卖方计算3代返佣 -2016-11-05
                $level_1_member_id= $this->setLevelMoney($trade_order['member_id']);
                $level_1_rate=0.1;
                if($level_1_member_id){
                    $r[] = $this->setUserMoney($level_1_member_id, 0, $trade_order_money * $level_1_rate, 'inc', 'num');
                    $r[] = $this->addFinance($level_1_member_id, 12, '推荐奖励',$trade_order_money * $level_1_rate, 1, 0);

                    $level_2_member_id= $this->setLevelMoney($level_1_member_id);
                    $level_2_rate=0.05;
                    if($level_2_member_id){
                        $r[] = $this->setUserMoney($level_2_member_id, 0, $trade_order_money * $level_2_rate, 'inc', 'num');
                        $r[] = $this->addFinance($level_2_member_id, 12, '推荐奖励',$trade_order_money * $level_2_rate, 1, 0);

                        $level_3_member_id= $this->setLevelMoney($level_2_member_id);
                        $level_3_rate=0.05;
                        if($level_3_member_id){
                            $r[] = $this->setUserMoney($level_3_member_id, 0, $trade_order_money * $level_3_rate, 'inc', 'num');
                            $r[] = $this->addFinance($level_3_member_id, 12, '推荐奖励',$trade_order_money * $level_3_rate, 1, 0);
                        }
                    }
                }
            ...
            case 'sell':
                ...
                //卖方计算3代返佣 -2016-11-05
                $level_1_member_id= $this->setLevelMoney($this->member['member_id']);
                $level_1_rate=0.1;
                if($level_1_member_id){
                    $r[] = $this->setUserMoney($level_1_member_id, 0, $order_money * $level_1_rate, 'inc', 'num');
                    $r[] = $this->addFinance($level_1_member_id, 12, '推荐奖励',$order_money * $level_1_rate, 1, 0);

                    $level_2_member_id= $this->setLevelMoney($level_1_member_id);
                    $level_2_rate=0.05;
                    if($level_2_member_id){
                        $r[] = $this->setUserMoney($level_2_member_id, 0, $order_money * $level_2_rate, 'inc', 'num');
                        $r[] = $this->addFinance($level_2_member_id, 12, '推荐奖励',$order_money * $level_2_rate, 1, 0);

                        $level_3_member_id= $this->setLevelMoney($level_2_member_id);
                        $level_3_rate=0.05;
                        if($level_3_member_id){
                            $r[] = $this->setUserMoney($level_3_member_id, 0, $order_money * $level_3_rate, 'inc', 'num');
                            $r[] = $this->addFinance($level_3_member_id, 12, '推荐奖励',$order_money * $level_3_rate, 1, 0);
                        }
                    }
                }
    ...
    //计算上级会员
    private function setLevelMoney($member_id)
    {
        ...
        return  $result_member['pid'];//将上级的id返回
